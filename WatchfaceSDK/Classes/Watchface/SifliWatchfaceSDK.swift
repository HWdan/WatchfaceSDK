...
@objc public class SifliWatchfaceSDK: NSObject {
    ...
    func pushAlbum(devIdentifier: String) {
        ...
        //开启推送
        if isBetterPhone() {
            self.pushManager.pushDialPlate(devIdentifier: devIdentifier, type: 3, zipPath: zipFilePath, maxFileSliceLength: 10 * 1024, withByteAlign: true)
        } else {
    ...
    func pushMusic(devIdentifier: String) {
        ...
        //开启推送
        if isBetterPhone() {
            self.pushManager.pushDialPlate(devIdentifier: devIdentifier, type: 4, zipPath: zipFilePath, maxFileSliceLength: 10 * 1024, withByteAlign: true)
        } else {
    ...
    private func doPushMapData(devIdentifier: String, zipFile: String) {
        ...
        //开启推送
        if isBetterPhone() {
            self.pushManager.pushDialPlate(devIdentifier: devIdentifier, type: 3, zipPath: URL(fileURLWithPath: zipFile), maxFileSliceLength: 10 * 1024, withByteAlign: false)
        } else {
    ...
    @objc public func setOnlineWatchface(devIdentifier: String, filePath: URL, progressCallback: @escaping QjsWatchfaceProgressCallback, finishCallback: @escaping QjsWatchfaceFinishCallback) {
        ...
        if isBetterPhone() {
            self.pushManager.pushDialPlate(devIdentifier: devIdentifier, type: 5, zipPath: filePath, maxFileSliceLength: 10 * 1024,withByteAlign: false)
        } else {
    ...
    @objc public func syncZipFile(devIdentifier: String, filePath: URL, type: Int, byteAlign: Bool, progressCallback: @escaping QjsWatchfaceProgressCallback, finishCallback: @escaping QjsWatchfaceFinishCallback) {
        ...
        //开启推送 在线表盘 withByteAlign 传 false
        if isBetterPhone() {
            self.pushManager.pushDialPlate(devIdentifier: devIdentifier, type: UInt16(type), zipPath: filePath, maxFileSliceLength: 10 * 1024,withByteAlign: byteAlign)
        } else {
    ...
    private func syncZipFile(devIdentifier: String, filePath: URL) {
        ...
        //开启推送
        if isBetterPhone() {
            self.pushManager.pushDialPlate(devIdentifier: devIdentifier, type: 5, zipPath: filePath, maxFileSliceLength: 10 * 1024, withByteAlign: true)
        } else {
